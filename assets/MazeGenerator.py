    ...
    for i in range(original_rows):
        for j in range(original_cols):
            if original_maze[i][j] == '0':
                ...
                if j < original_cols - 1 and original_maze[i][j + 1] == '0':  # Right neighbor
                    new_maze[i * 3 + 1][j * 3 + 3] = '0'  # Connect to the right
                    new_maze[i * 3 + 2][j * 3 + 3] = '0'
                if i < original_rows - 1 and original_maze[i + 1][j] == '0':  # Bottom neighbor
...
def add_random_crosswalks(maze, num_crosswalks):
    for _ in range(num_crosswalks):
        ...
        if maze[y][x] == '2' and maze[y][x + 1] == '0' and maze[y][x + 2] == '0' and maze[y][x + 3] == '2' and maze[y+1][x + 1] == '0' and maze[y+1][x + 2] == '0' and maze[y+1][x + 3] == '2' and maze[y-1][x + 1] == '0' and maze[y-1][x + 2] == '0' and maze[y-1][x + 3] == '2':
            maze[y][x + 1] = '3'
            maze[y][x + 2] = '3'
        elif maze[y][x] == '2' and maze[y + 1][x] == '0' and maze[y + 2][x] == '0' and maze[y + 3][x] == '2' and maze[y+1][x + 1] == '0' and maze[y+2][x + 1] == '0' and maze[y+3][x + 1] == '2' and maze[y+1][x - 1] == '0' and maze[y+2][x - 1] == '0' and maze[y+3][x - 1] == '2':
            maze[y + 1][x] = '3'
            maze[y + 2][x] = '3'
    return maze

def check_if_neighboor_is_crosswalk_or_sidewalk(maze, x, y):
    if maze[y-1][x] == '3' or maze[y-1][x] == '2' or maze[y+1][x] == '3' or maze[y+1][x] == '2' or maze[y][x-1] == '3' or maze[y][x-1] == '2' or maze[y][x+1] == '3' or maze[y][x+1] == '2':
        return True
    return False
...
def add_start_end(maze):
    ...
    while new:
        for tile in connected_tiles:
            ...
            y = tile[0]
            new = False
            if check_if_it_is_a_walkable_tile(maze, x, y-1) and not (y-1, x) in connected_tiles:
                connected_tiles.append((y-1, x))
                new = True
            if check_if_it_is_a_walkable_tile(maze, x, y+1) and not (y+1, x) in connected_tiles:
                connected_tiles.append((y+1, x))
                new = True
            if check_if_it_is_a_walkable_tile(maze, x-1, y) and not (y, x-1) in connected_tiles:
...
def add_random_decorations(maze, decorations):
    for _ in range(decorations):
        ...
        if maze[y][x] == '1':
            ...
            if maze[y][x + 1] == '1' and maze[y + 1][x] == '1' and maze[y + 1][x + 1] == '1':
                # i need to check if i'm not too close to another decoration
                if maze[y-1][x] not in decorations_types and maze[y+1][x] not in decorations_types and maze[y][x-1] not in decorations_types and maze[y][x+1] not in decorations_types and maze[y-1][x-1] not in decorations_types and maze[y-1][x+1] not in decorations_types and maze[y+1][x-1] not in decorations_types and maze[y+1][x+1] not in decorations_types:
                    maze[y][x] = str(type)
    return maze
